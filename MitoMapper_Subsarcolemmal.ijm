...
//setup and load all the images
	...
	SSL= floor(0.3/pixelWidth); // subsarcolemmal search depth in micrometer
	SSLOF= 25; //subsarcolemmal weighting factor 0.01 sensitive; 256 no subsarcolemmal population; 25 aprox. 10% 
	...
	setThreshold(255, 255);
	run("Convert to Mask");
// optional file load ins 
	maskPath = dir + "/Nucleolus_Masks/" + name + "_nucleolus.tif";
...
// optional nucleolus positioning
if (nucleolus== true) {
	roiManager("reset");
	run("Clear Results");
	...
	if (nn>1) {
		run("Summarize");
		noArea=(getResult("Area", n)*ASF);
		noCirc=getResult("Circ.", n);
		noAR=getResult("AR", n);
		noRound=getResult("Round", n);
		noSolidity=getResult("Solidity", n);
	}
	else {
		noArea=(getResult("Area", 0)*ASF);
		noCirc=getResult("Circ.", 0);
		noAR=getResult("AR", 0);
		noRound=getResult("Round", 0);
		noSolidity=getResult("Circ.", 0);
	}
	XN= newArray();
	YN= newArray();
	for (i = 0; i < nn; i++) {
	...
	NoP=newArray();
		for (q = 0; q < nn; q++) {
			roiManager("reset");
			selectWindow("Nucleus");
			imageCalculator("Subtract create", "Nucleus","Nucleolus");
...
//Nucleolus positioning
	...
	for (x = 0; x < 180; x++) {
		...
		Array.getStatistics(NU, NUmin, NUmax, NUmean, NUstdDev);
		WNO[x]= (NUmax-NUmin);
	}
	Array.getStatistics(WNO, WNOmin, WNOmax, WNOmean, WNOstdDev);
	NoP[w]= (WNOmin*pixelWidth);
	wait(500);
	}
	Array.getStatistics(NoP, NoPmin, NoPmax, NoPmean, NoPstdDev);
	if (ts==true) {
	print(NoPmin, noArea, noCirc, noAR, noRound, noSolidity);
	}
	close("Nucleolus");
	close("Result of Nucleolus");
}
// End of Extr Nuclear add in section
...
SSLPi=0;
SSLP=newArray();
// subsarcollemal population association
selectImage("Cell");
run("Duplicate...", "title=Cell-Invert");
run("Invert");
for (i = 0; i < SSL; i++) {
	run("Dilate");
	if (i/10 == floor(i/10)){
	imageCalculator("Subtract create", "Cell-Invert","Myo");	
	close("Cell-Invert");
	selectImage("Result of Cell-Invert");
	rename("Cell-Invert");
}
}
imageCalculator("Subtract create", "Cell-Invert","Myo");
run("Clear Results");
selectImage("Mito");
run("Set Measurements...", "area mean standard center shape redirect=Cell-Invert decimal=3");
run("Analyze Particles...", "display add");
roiManager("reset");
...
SSLPLMT = extractValues(LMT, SSLP);
SSLPLMT[0]="Length";
SSLPWMT = extractValues(WMT, SSLP);
SSLPWMT[0]= "Width";
SSLPmA  = extractValues(mA,  SSLP);
SSLPmA[0]= "Area";
SSLPmC  = extractValues(mC,  SSLP);
SSLPmC[0]= "circularity";
SSLPmAR = extractValues(mAR, SSLP);
SSLPmAR[0]= "Aspect Ratio";
SSLPmRO = extractValues(mRO, SSLP);
SSLPmRO[0]= "Roundness";
SSLPmSO = extractValues(mSO, SSLP);
SSLPmSO[0]= "Solidity";
...
if (nucleolus==true) {
	print("Nucleuolus Area" ,",", "Nucleolus Circ." ,",", "Nucleolus AR" ,",", "Nucleolus Roundness" ,",", "Nucleolus Solidity",",", "Nucleolus Distance to Nuclear Membrane",",", "Number of Nucleoli Found");
	print(noArea,",", noCirc ,",", noAR ,",", noRound ,",", noSolidity,",", NoPmin,",", nn);
}
print("Close mitos");
Array.print(CLMT);
Array.print(CWMT);
